...
@section('js')
  ...
  // Máscara para CPF
    ...
    // Validação de CPF e Celular ao perder o foco
    $('#cpf').on('blur', function () {
        const valor = $(this).val();
        const isValid = valor.length === 14 || valor.length === 0; // Permite campo vazio
    ...
    // Impede envio com erros apenas se os campos estiverem preenchidos e inválidos
    $('#form').on('submit', function(e) {
        ...
        if (cel.length > 0 && (cel.length < 10 || cel.length > 11)) {
            $('#celular').addClass('is-invalid');
            hasError = true;
        }

        if (hasError) {
            e.preventDefault(); // Impede o envio apenas se houver erro
        }
    });
