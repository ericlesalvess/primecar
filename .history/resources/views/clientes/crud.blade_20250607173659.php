    ...
    <div class="card card-primary">
        ...
        <div class="card-body"s>
            <div class="form-group">
                ...
                @if (isset($edit->id))
                    <form id = "form" method="post" action="{{ route('cliente.update', ['cliente' => $edit->id]) }}">
                        @csrf
                        @method('PUT')
                    @else
                        <form id = "form" method="post" action="{{ route('cliente.store') }}">
                            @csrf
                @endif
                    ...
                    <div class="col-sm-4">
                        ...
                        <input type="text" class="form-control" id="celular" name="celular" placeholder=""
                            ...
                            <div class="invalid-feedback">
                                Número de celular deve ter 10 ou 11 dígitos.
                            </div>
...
@section('js')
    ...
    $('#cpf').on('blur', function () {
        const isValid = $(this).val().length === 14;
        $(this).toggleClass('is-invalid', !isValid);
    });

    // Validação visual para celular
    $('#celular').on('blur', function () {
        const length = $(this).val().replace(/\D/g, '').length;
        const isValid = length >= 10 && length <= 11;
        $(this).toggleClass('is-invalid', !isValid);
    });
    ...
    $('#form').on('submit', function(e) {
        ...
        if (cpf.length !== 14) {
            ...
            $('#cpf').removeClass('is-invalid').addClass('is-valid'); // Remove erro ao corrigir
        }
        ...
        if (cel.length < 10 || cel.length > 11) {
            ...
            $('#celular').removeClass('is-invalid').addClass('is-valid'); // Remove erro ao corrigir
        }
    ...
    // Remover erro automaticamente quando o usuário começa a digitar
    $('#cpf, #celular').on('input', function() {
        $(this).removeClass('is-invalid is-valid'); // Remove estilos ao digitar
    });
});

        //Bloqueia envio de formulário caso CPF ou celular estejam incompletos
        $('#form').submit(function(event) {
            let cpfVal = $('#cpf').val();
            let celVal = $('#celular').val().replace(/\D/g, ''); // Remove caracteres não numéricos
                 if (cpfVal.length !== 14 || (celVal.length !== 10 && celVal.length !== 11)) {
                    alert("Preencha corretamente os campos de CPF e elular.");
